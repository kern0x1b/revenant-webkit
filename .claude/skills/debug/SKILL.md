...
1. `make device ARGS='run 12 "cat /tmp/rev-safari-tweak.log"'` — is there a
...
2. Confirm the two dylibs are the right files and sizes:
   `make device ARGS='run 12 "ls -la /Library/MobileSubstrate/DynamicLibraries/RevSafari.dylib /usr/lib/rev-safari-compat.dylib"'`.
...
```sh
make device ARGS='run 15 "cp -f /Library/MobileSubstrate/DynamicLibraries/RevSafari.dylib.bak2 /Library/MobileSubstrate/DynamicLibraries/RevSafari.dylib; killall SpringBoard"'
```
