...
S=$P/webkit-254; B=$P/build-254-slim
...
cmake -S $S -B $B -G Ninja \
  ...
  -DPORT=IOS -DCMAKE_BUILD_TYPE=Release -DCMAKE_CXX_FLAGS_RELEASE="-O2 -DNDEBUG" -DCMAKE_C_FLAGS_RELEASE="-O2 -DNDEBUG" -DCMAKE_OBJC_FLAGS_RELEASE="-O2 -DNDEBUG" -DCMAKE_OBJCXX_FLAGS_RELEASE="-O2 -DNDEBUG" -DDEVELOPER_MODE=OFF \
  ...
  -DENABLE_SAMPLING_PROFILER=OFF -DENABLE_MEDIA_SOURCE=OFF -DENABLE_MEDIA_SOURCE_IN_WORKERS=OFF -DENABLE_ENCRYPTED_MEDIA=OFF -DENABLE_LEGACY_ENCRYPTED_MEDIA=OFF -DENABLE_WEB_AUTHN=OFF -DENABLE_WRITING_TOOLS=OFF -DENABLE_PAYMENT_REQUEST=OFF -DUSE_SYSTEM_MALLOC=ON \
